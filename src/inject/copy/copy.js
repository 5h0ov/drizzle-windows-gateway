...
(() => {
      ...
      if (rect && rect.width > 0) {
        ...
        const probeEl =
          (root.elementFromPoint
            ? root.elementFromPoint(probeX, probeY)
            : null) || document.elementFromPoint(probeX, probeY);
        if (probeEl) {
          const found = probeEl.closest
            ? probeEl.closest('[role="row"]')
            : null;
          if (found) {
            return found;
          }
  ...
  function getFullRecordFromRow(rowEl, _root, allGridRows) {
    if (!rowEl) {
      return null;
    }
    ...
    try {
      ...
      if (rowIdxAttr && allGridRows && allGridRows.length > 0) {
        ...
        const rowIdx = Number.parseInt(rowIdxAttr, 10) - 2;
    ...
    const candidates = [
      ...
      rowEl.querySelector('[role="gridcell"]'),
    ];
            ...
            if (props.row) {
              ...
              if (
                ...
                (props.row.id !== undefined ||
                  Object.keys(props.row).length > 2)
              ) {
                return props.row;
              }
            }
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      for (const p of path) {
        const found = findRowFromElement(p);
        if (found) {
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      for (const el of path) {
        ...
        if (txt.includes("Copy as JSON")) {
          actionType = "copy_json";
          break;
        }
        if (txt.includes("Copy as CSV")) {
          actionType = "copy_csv";
          break;
        }
        if (txt.includes("Copy as SQL")) {
          actionType = "copy_sql";
          break;
        }
      }
      ...
      const checkedRows = Array.from(
        root.querySelectorAll('[role="row"][aria-selected="true"]')
      );
      let rowsToProcess = [];
      if (
        checkedRows.length > 0 &&
        lastRightClickedRow &&
        checkedRows.includes(lastRightClickedRow)
      ) {
      ...
      const headerRow =
        root.querySelector('[role="row"][aria-rowindex="1"]') ||
        root.querySelector(".rdg-header-row");
      ...
      for (const hc of headerCells) {
        ...
        const name = (
          nameSpan ? nameSpan.textContent : hc.textContent || ""
        ).trim();
        if (name) {
          domColNames.push(name);
        }
      }

      // Process rows using React Fiber (for 100% complete schema columns), fallback to DOM cells
      const parsedRecords = [];
      for (const rowEl of rowsToProcess) {
        ...
        if (fiberValues && typeof fiberValues === "object") {
          ...
          const gridCells = Array.from(
            rowEl.querySelectorAll('[role="gridcell"]')
          );
          ...
          const obj = {};
          domColNames.forEach((colName, idx) => {
      ...
      if (actionType === "copy_all") {
        ...
        const csvLines = parsedRecords.map((r) =>
          sampleCols
            .map((k) => {
              const v = r[k];
              const strVal = formatCsvVal(v);
        ...
        const tableHeader = root.querySelector(
          'h1, [data-testid="table-name"], .truncate.font-medium'
        );
        const tableName = tableHeader
          ? tableHeader.textContent.trim()
          : "table_name";
        textToCopy = parsedRecords
          .map((r) => {
            const cols = sampleCols.join(", ");
            const vals = sampleCols
              .map((k) => {
                const v = r[k];
                if (v === null || v === undefined) {
                  return "NULL";
      ...
      if (textToCopy) {
        ...
        showCopyToast(
          parsedRecords.length === 1
            ? "Copied 1 row"
            : `Copied ${parsedRecords.length} rows`
        );
      }
    },
    true
