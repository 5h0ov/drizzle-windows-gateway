...
(() => {
  if (initConn) {
    ...
    !(
      sessionStorage.getItem("drizzle_window_conn") ||
      sessionStorage.getItem("drizzle_window_empty")
    )
  ) {
        ...
        if (parsed?.state) {
          if (parsed.state.currentConnection) {
            sessionStorage.setItem(
              "drizzle_window_conn",
              JSON.stringify(parsed.state.currentConnection)
            );
          } else {
            sessionStorage.setItem("drizzle_window_empty", "true");
          }
  ...
  if (targetObj) {
    ...
    const timer = setInterval(() => {
      ...
      const match = all.find(
        (el) => el.textContent && el.textContent.trim() === targetObj.name
      );
      if (match) {
        match.click();
        clearInterval(timer);
    ...
    const timer = setInterval(() => {
      ...
      const all = Array.from(document.querySelectorAll("button, div, span, a"));
      const backBtn = all.find(
        (el) =>
          el.textContent && el.textContent.trim() === "Back to connections"
      );
      if (backBtn) {
        backBtn.click();
        clearInterval(timer);
